 ...
 *      Author: user
 */
//banktb feh implementation beta3 ay argument
...
void DIO_voidSetPinDir(u8 Copy_u8Port,u8 Copy_u8Pin,u8 Copy_u8Dir){
	//Copy_u8Dir==0 input else 1 hayb2a output
	if(Copy_u8Dir==DIO_u8INPUT){
		switch(Copy_u8Port)
		{
			//hena ClR_BIT 3ashen 3yz a7ot 0 3ashen tb2a input
		...
		switch(Copy_u8Port){
			//set_bit 3ashen na7ot el bit 1 fa tb2a output
...
u8 DIO_u8GetPinValue(u8 Copy_u8Port , u8 Copy_u8Pin){
	u8 Local_u8Result;
	switch(Copy_u8Port){
		//pina dh register el bnakhod meno el input value
		// GET_BIT(PINA,Copy_u8Pin);ba2olo hatly el value el fe pin kaza men register PINA we hkza ba2a
